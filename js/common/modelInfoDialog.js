...
export class ModelInfoDialog extends ComfyDialog {
	constructor(name, node) {
		...
		this.name = name;
		this.node = node;
		this.element.classList.add("weilin-autocom-model-info");
	}

	get customNotes() {
		return this.metadata["weilin_autocom.notes"];
	}

	set customNotes(v) {
		this.metadata["weilin_autocom.notes"] = v;
	}

	get hash() {
		return this.metadata["weilin_autocom.sha256"];
	}

	async show(type, value) {
		this.type = type;

		const req = api.fetchApi("/weilin/autocomplete/metadata/" + encodeURIComponent(`${type}/${value}`));
		this.info = $el("div", { style: { flex: "auto" } });
		this.img = $el("img", { style: { display: "none" } });
		this.imgWrapper = $el("div.weilin-autocom-preview", [this.img]);
		this.main = $el("main", { style: { display: "flex" } }, [this.info, this.imgWrapper]);
		this.content = $el("div.weilin-autocom-model-content", [$el("h2", { textContent: this.name }), this.main]);
	...
	getNoteInfo() {
		...
		const edit = $el("a", {
			...
			onclick: async (e) => {
				...
				if (textarea) {
					...
					const resp = await api.fetchApi("/weilin/autocomplete/metadata/notes/" + encodeURIComponent(`${this.type}/${this.name}`), {
						method: "POST",
					...
					this.node?.["weilin_autocom.updateExamples"]?.();
				} else {
		...
		notesContainer = $el("div.weilin-autocom-model-notes", parseNote.call(this));
		return $el(
			"div",
			{
				...
				if (previews?.length) {
					...
					this.imgSave = $el("button", {
						...
						onclick: async () => {
							...
							// Use as preview
							await api.fetchApi("/weilin/autocomplete/save/" + encodeURIComponent(`${this.type}/${this.name}`), {
								method: "POST",
								body: JSON.stringify({
									filename: name,
									type: "temp",
					...
					const addNavButton = (icon, direction) => {
						$el("button.weilin-autocom-preview-nav", {
							textContent: icon,
							parent: this.imgWrapper,
							onclick: async () => {
