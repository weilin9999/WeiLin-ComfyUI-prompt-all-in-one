...
async function getCustomWords() {
	const resp = await api.fetchApi("/weilin/autocomplete", { cache: "no-store" });
	if (resp.status === 200) {
...
class CustomWordsDialog extends ComfyDialog {
	...
	createButtons() {
		...
		const save = $el("button", {
			...
			onclick: async (e) => {
				try {
					const res = await api.fetchApi("/weilin/autocomplete", { method: "POST", body: this.words.value });
					if (res.status !== 200) {
...
const id = "weilin_autocom.AutoCompleter";
...
function initComplete() {
	...
	ComfyWidgets.STRING = function (node, inputName, inputData) {
		...
		if (inputData[1]?.multiline) {
			...
			const config = inputData[1]?.["weilin_autocom.autocomplete"];
...
app.registerExtension({
	...
	setup() {
		async function addEmbeddings() {
			...
			TextAreaAutoComplete.updateWords("weilin_autocom.embeddings", words);
		}
		...
		async function addLoras() {
			...
			if (!loras?.length) {
				loras = await api.fetchApi("/weilin/autocomplete/loras", { cache: "no-store" }).then((res) => res.json());
			}
			...
			TextAreaAutoComplete.updateWords("weilin_autocom.loras", words);
		}
	...
	beforeRegisterNodeDef(_,nodeData, def) {
		...
		for (const input in inputs) {
			const config = inputs[input][1]?.["weilin_autocom.autocomplete"];
			if (!config) continue;
